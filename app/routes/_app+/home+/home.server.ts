...
export async function buscaResultadosQuizzesUltimos6M(
	userId: string,
): Promise<{ mesAno: string; total: number; corretas: number }[]> {
	...
	return result.map(value => {
		...
		return {
			mesAno: `${getMonthName(value.mes_ano.getUTCMonth() + 1)}/${ano.toString()}`,
...
export async function buscaResultadosMesAtual(userId: string) {
	const result = await prisma.$queryRaw<{ total: number; corretas: number }[]>`

...
export async function loader({ params }: LoaderFunctionArgs) {
	...
	const notify = await prisma.notifyError.findFirst({
		select: {
			id: true,
			fixed: true,
			fixedMessage: true,
...
export async function action({ request, params }: ActionFunctionArgs) {
	...
	const submission = parseWithZod(formData, { schema: schemaForm })
	if (submission.status !== 'success') {
		return json({ result: submission.reply() }, { status: 400 })
	}

	await prisma.notifyError.update({
		where: { id: notifyId },
		data: {
			fixed: true,
			fixedById: userId,
			fixedMessage: submission.value.fixedMessage,
		},
	})
	const headers = await createToastHeaders({
		description: 'Notificação marcada como resolvida com sucesso',
	})
	return json({ result: 'success' }, { headers })
}
	...
	return (
		<div className="space-y-5">
			...
			{notify.quiz ? (
				<div className="space-y-2">
					...
					<div>
						<div>
							...
							<div className="rounded-md border p-2 shadow-md">
								<p
									dangerouslySetInnerHTML={{ __html: notify.quiz.enunciado }}
								/>
							</div>
						</div>
						<div>
							...
							<div className="rounded-md border p-2 shadow-md">
								<p
									dangerouslySetInnerHTML={{ __html: notify.quiz.comentario }}
								/>
							</div>
						</div>
				...
				<div>
					<div className="flex items-center space-x-4">
						<h2 className="h-full font-semibold">Informações do Flashcard</h2>
						<Link
							to={`/admin/flashcards/${notify.flashcard.id}/edit`}
							target="_black"
						>
							<Button>Abrir Flashcard</Button>
						</Link>
					</div>
					...
					<div>
						Matéria:{' '}
						<span className="font-semibold">
							{notify.flashcard.artigo.capitulo.titulo.lei.materia.name}
						</span>
					</div>
					<div>
						Lei:{' '}
						<span className="font-semibold">
							{notify.flashcard.artigo.capitulo.titulo.lei.name}
						</span>
					</div>
					<div>
						Título:{' '}
						<span className="font-semibold">
							{notify.flashcard.artigo.capitulo.titulo.name}
						</span>
					</div>
					...
					<div>
						Capítulo:{' '}
						<span className="font-semibold">
							{notify.flashcard.artigo.capitulo.name}
						</span>
					</div>
					<div>
						Artigo:{' '}
						<span className="font-semibold">
							{notify.flashcard.artigo.name}
						</span>
					</div>
					<div>
						<div>
							...
							<div className="rounded-md border p-2 shadow-md">
								<p
									dangerouslySetInnerHTML={{ __html: notify.flashcard.frente }}
								/>
							</div>
						</div>
						<div>
							...
							<div className="rounded-md border p-2 shadow-md">
								<p
									dangerouslySetInnerHTML={{ __html: notify.flashcard.verso }}
								/>
							</div>
						</div>
						{notify.flashcard.fundamento ? (
							<div>
								<h3 className="font-semibold">Fundamento:</h3>
								<div className="rounded-md border p-2 shadow-md">
									<p
										dangerouslySetInnerHTML={{
											__html: notify.flashcard.fundamento,
										}}
									/>
								</div>
							</div>
			...
			<div>
				...
				{notify.fixedBy ? (
					<div>
						...
						<h3 className="font-semibold">Mensagem da correção:</h3>
						<div className="rounded-md border p-2 shadow-md">
					...
					<>
						<Form method="post" {...getFormProps(form)}>
							<TextareaField
								labelProps={{
									children:
										'Descreva a solução abaixo para marcar como resolvido',
								}}
								textareaProps={{
									...getInputProps(fields.fixedMessage, { type: 'text' }),
								}}
								errors={fields.fixedMessage.errors}
							/>
